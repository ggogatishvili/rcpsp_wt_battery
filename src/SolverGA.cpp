...
Solution SolverGA::_solve(const bool lp_batt_eval)
{
    ...
    try {
        ...
        try {
            ...
            fmt::println(stderr, "GA fallback to H1 also failed ({}). "
...
eoPop<Chromosome> SolverGA::generateInitialPopulation() const
{
    ...
    // as an infeasible individual (fitness -BIG_M, see ga/Evaluator.h); seeding
    // must degrade the same way. Letting the exception escape here aborted the
    // entire run before the search started, which deterministically killed
    // ~6% of GA and ~7% of GAP runs on short-horizon instances -- and did so on
    // every seed, since these orderings do not consult the RNG.
    {
        SolverH1 seeder(ins);
        ...
        auto seedWith = [&](const vector<double>& prios, const bool priceAware) {
            try {
                ...
                // population is topped up with random chromosomes below, so
                // its size is unaffected.
                ++skippedSeeds;
            }
        };
        ...
        if (Config::verbose && skippedSeeds > 0)
            fmt::println("GA: {} of 6 heuristic seed(s) skipped "
